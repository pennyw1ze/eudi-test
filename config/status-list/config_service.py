...
class ConfService:
    ...
    # Overridden from upstream's "https://localhost/".
    #
    # This value is baked into the status list URIs the service hands out. Those URIs are
    # fetched by the issuer and the verifier, which run in containers where "localhost"
    # is the container itself, not the gateway. Using the compose service name makes the
    # URIs resolvable by every consumer that actually dereferences them.
    service_url = "http://status-list:5000/"
